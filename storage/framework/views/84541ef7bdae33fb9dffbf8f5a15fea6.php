...
<head>
    <meta charset="UTF-8">
    <title>Elite PVP Arena | <?php echo e($room->room_code); ?></title>
    ...
    <style>
        ...
        /* 🏆 MAIN BATTLE STATION (Fixed Height to prevent button jumping) */
        .battle-card { 
        ...
        /* 🏎️ SCROLLABLE OPTIONS (Strike Button Visibility Fix) */
        #qArea { flex-grow: 1; display: flex; flex-direction: column; overflow: hidden; }
        #optionsGrid { 
            overflow-y: auto; flex-grow: 1; max-height: 260px; padding-right: 8px; margin-bottom: 10px;
        }
        ...
        /* 🔒 BUTTON LOCK (Heal Drawback) */
        .strike-btn-locked { background: #334155 !important; border: none !important; opacity: 0.4 !important; pointer-events: none !important; cursor: not-allowed; color: #94a3b8 !important; }
        ...
        .rank-name.surrendered { text-decoration: line-through; color: #ef4444; opacity: 0.5; }
        ...
        .is-frozen-state #frozenOverlay { display: flex; pointer-events: auto; }
    ...
    <div class="row g-3">
        <div class="col-lg-8">
            <div class="battle-card" id="arenaCard">
                <div id="frozenOverlay" class="game-overlay"><h1 class="display-2 fw-black text-info">FROZEN</h1><div id="freezeCountdown" class="fs-2 fw-bold text-white"></div></div>

                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h3 class="text-warning fw-black mb-0"><?php echo e(strtoupper(Auth::user()->name)); ?></h3>
                    <div class="text-end">
                        <a href="<?php echo e(route('student.quizzes.pvp.surrender', $room->room_code)); ?>" class="btn btn-outline-danger btn-sm rounded-pill px-3 py-0 mb-1 fw-black" style="font-size: 0.7rem;">SURRENDER</a>
                        <div class="progress mb-1" style="width: 180px; height: 16px; border-radius: 50px; background: rgba(0,0,0,0.5);"><div id="myHp" class="hp-fill"></div></div>
                        <div class="progress" style="width: 180px; height: 8px; border-radius: 50px; background: rgba(0,0,0,0.5);"><div id="myMp" class="mp-fill"></div></div>
                    </div>
                </div>
        ...
        <div class="col-lg-4">
            <div class="p-3 bg-dark rounded-4 border border-secondary h-100 shadow-lg">
                <h6 class="text-primary mb-3 fw-black"><i class="fas fa-bolt me-2"></i>RANKINGS</h6>
...
<script>
    // Configuration & State
    const roomCode = "<?php echo e($room->room_code); ?>";
    const baseURL = "/student/quizzes/pvp/<?php echo e($room->room_code); ?>";
    const resultsURL = "<?php echo e(route('student.quizzes.pvp.results', $room->room_code)); ?>";
    
    let quizBank = <?php echo json_encode($room->quiz->questions, 15, 512) ?>; 
    let currentIdx = 0, timer, timeLeft = 60, selectedIds = [];
    let isFrozen = false, feedbackActive = false, spellUsedThisTurn = false;

    // Fetch next question or reshuffle if bank is empty
    function getQ() { 
    ...
    /**
     * 🏁 HEARTBEAT & SYNC
     * Updates HP/MP and checks for Victory conditions every 1.5 seconds.
     */
    async function sync() {
        try {
            ...
            // 1. CRITICAL: INSTANT VICTORY REDIRECT
            // If server says finished, we leave immediately.
            if (data.status === 'finished' && !feedbackActive) { 
                window.location.href = resultsURL; 
                return; 
            }
            
            const me = data.participants.find(p => p.user_id == <?php echo e(Auth::id()); ?>);
            if (!me) return;

            const arena = document.getElementById('arenaCard');

            // 🎨 Themes & Glowing Effects
            arena.classList.toggle('theme-shield', me.is_shielded);
            arena.classList.toggle('theme-boost', me.active_boost);

            // 🔒 HEAL DRAWBACK: Hard block the Execute button
            const sBtn = document.getElementById('submitBtn');
            if (me.strike_locked) {
                sBtn.classList.add('btn-locked');
                sBtn.disabled = true; // Hard lock
                sBtn.innerText = "RECOVERING...";
            } else if (!feedbackActive) {
                sBtn.classList.remove('btn-locked');
                sBtn.disabled = false;
                sBtn.innerText = "EXECUTE STRIKE ⚡";
            }

            // ❄️ Frozen State Overlay
            if (me.is_frozen) {
                isFrozen = true; 
            ...
            // 📊 HUD Progress Bars
            const maxHp = data.participants.length * 50; // Dynamic scaling
            document.getElementById('myHp').style.width = (me.hp / maxHp * 100) + "%";
            document.getElementById('myMp').style.width = me.mp + "%";
            
            // 🛡️ Ability Button Logic (Lock if Shield drawback is active)
            ['heal', 'shield', 'freeze', 'boost'].forEach(p => {
                const btn = document.getElementById(`p-${p}`);
                const cost = parseInt(btn.innerText.match(/\d+/)[0]);
                const isLocked = me.skills_locked || spellUsedThisTurn;
                btn.classList.toggle('active', me.mp >= cost && !isLocked);
            });

            // 🏆 Live Rankings (with Surrender Cross-out)
            document.getElementById('warriorList').innerHTML = data.participants.map(p => `
                <div class="mb-3">
                    <div class="d-flex justify-content-between small fw-bold">
                        <span class="${p.status === 'surrendered' ? 'rank-name surrendered' : ''}">
                            ${p.name} ${p.hp <= 0 && p.status !== 'surrendered' ? '💀' : ''}
                        </span>
                        <span>${p.hp > 0 ? p.hp : '0'} HP</span>
                    </div>
                    ${p.status === 'surrendered' ? 
                        '<div class="surrender-label mt-1">SURRENDERED</div>' : 
    ...
    function handleSel(id, type, btn) {
        if (isFrozen || feedbackActive) return;
        if (type === 'single') { 
    ...
    async function submitAns(isTimeout = false) {
        ...
        const q = getQ(), sBtn = document.getElementById('submitBtn'); 
        sBtn.disabled = true;
        ...
        try {
            ...
            // Instant Correction Visuals
            if (q.question_type === 'text') {
                const el = document.getElementById('ansInput');
                el.classList.add(result.is_correct ? 'correct' : 'incorrect');
                if(!result.is_correct) el.value = "WRONG!";
            } else {
                document.querySelectorAll('.option-card').forEach(btn => {
                    const id = parseInt(btn.dataset.id);
                    if (result.correct_options?.includes(id)) btn.classList.add('correct');
                    else if (selectedIds == id || (Array.isArray(selectedIds) && selectedIds.includes(id))) btn.classList.add('incorrect');
                });
            }

            // 🏎️ 1-SECOND RELOAD: Fast transition to next turn
            setTimeout(() => { currentIdx++; renderQ(); }, 1000);
        } catch (e) { renderQ(); }
    }
...
</html><?php /**PATH C:\laragon\www\islamicWebsiteEducation_FYP_2023657278\resources\views/users/quizzes/pvp_arena.blade.php ENDPATH**/ ?>