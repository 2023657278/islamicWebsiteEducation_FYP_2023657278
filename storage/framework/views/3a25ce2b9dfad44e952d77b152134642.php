...
<head>
    ...
    <title>Mission Lobby | <?php echo e($room->room_code); ?></title>
...
<div class="lobby-container shadow-lg">
    ...
    <div class="room-badge mb-4 shadow"><?php echo e($room->room_code); ?></div>
    ...
    <div class="mt-5 pt-4 border-top">
        ...
        <?php else: ?>
            ...
            <a href="<?php echo e(route('student.quizzes.lobby.leave', $room->room_code)); ?>" class="btn btn-outline-secondary rounded-pill px-4">LEAVE ARENA</a>
        <?php endif; ?>
    </div>
</div>
...
<script>
    ...
    async function confirmDismiss() {
        if(confirm("Abort mission? This will close the lobby for everyone.")) {
            window.onbeforeunload = null; 
            const res = await fetch(`/student/quizzes/lobby/${roomCode}/dismiss`, { method: 'POST', headers: {'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'} });
    ...
    function updateLobby() {
        fetch(`/student/quizzes/lobby/${roomCode}/participants`)
            .then(res => res.json())
            .then(data => {
                if (data.status === 'dismissed' || !data.participants) { window.location.href = "<?php echo e(route('student.quizzes.index')); ?>"; return; }
