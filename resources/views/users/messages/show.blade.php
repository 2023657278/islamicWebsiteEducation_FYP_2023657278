...
@section('content')
<style>
    /* ✅ WhatsApp-inspired container with refined border */
    ...
    /* ✅ FIXED: Header matching your high-end reference */
    ...
    /* ✅ FIXED: Rounded Square Avatar */
    .wa-chat-avatar { 
        width: 45px; height: 45px; 
        background: #E6FFFA; /* Teal reference background */
        color: #008f78; /* Deep teal reference text */
        border-radius: 12px; /* Professional rounded square */
        overflow: hidden; 
        margin-right: 15px; 
        display: flex; 
        align-items: center; 
        ...
        <div class="col-md-10 col-lg-8">
            <div class="wa-chat-container shadow-sm">
                ...
                <div class="wa-chat-header">
                    <a href="{{ route('student.messages.index') }}" class="wa-back-btn">
                        <i class="fas fa-arrow-left"></i>
                    </a>
                    
                    <div class="wa-chat-avatar">
                        @if($isBroadcast)
                            <i class="fas fa-bullhorn" style="font-size: 1.2rem; color: #8B1E24;"></i>
                        @elseif($teacher->profile_image)
                            <img src="{{ asset('storage/profile_images/' . $teacher->profile_image) }}" alt="Profile">
                        @else
                            {{-- Matching reference fallback style --}}
                        ...
                        <div class="wa-bubble {{ ($message->sender_id == Auth::id()) ? 'wa-sent' : 'wa-received' }}">
                            ...
                            <div class="wa-meta">
                                ...
                                @if($message->sender_id == Auth::id())
                                    <i class="fas fa-check-double ms-1 text-primary" style="font-size: 10px;"></i>
                                @endif
                            </div>
                        </div>
                    @endforeach
                ...
                @else
                    <div class="wa-input-area">
                        ...
                        <form id="chatForm" action="{{ route('student.messages.store', $teacher->id) }}" method="POST" style="flex: 1; display: flex;">
                            @csrf
                            <input type="text" name="message" class="wa-input-field" placeholder="Type a message..." autocomplete="off" required>
                        </form>

                        <button class="wa-icon-btn" onclick="document.getElementById('chatForm').submit()">
