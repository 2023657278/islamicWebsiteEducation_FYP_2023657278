...
<div class="container d-flex justify-content-center align-items-center" style="min-height: 70vh;">
    <div class="card border-0 shadow rounded-4 text-center p-5" style="max-width: 450px; width: 100%;">
        ...
        <h2 class="fw-bold text-dark mb-1">
            @if($percentage >= 50) Alhamdulillah! @else Keep Trying! @endif
        </h2>
        <p class="text-muted mb-4">You've completed the quiz.</p>

        <div class="bg-light rounded-4 p-4 mb-4 border {{ $percentage >= 50 ? 'border-success' : 'border-danger' }} border-opacity-25">
            <h1 class="display-3 fw-bold {{ $percentage >= 50 ? 'text-success' : 'text-danger' }} mb-0">{{ $percentage }}%</h1>
            <p class="text-muted mb-0">{{ $score }} out of {{ $totalQuestions }} correct answers</p>
        </div>

        <div class="d-grid gap-2">
            <a href="{{ route('student.quizzes.take', $quiz->id) }}" class="btn btn-outline-dark rounded-pill fw-bold">Retake Quiz</a>
            <a href="{{ route('student.quizzes.index') }}" class="btn btn-primary rounded-pill fw-bold">Back to Quiz Center</a>
        </div>
