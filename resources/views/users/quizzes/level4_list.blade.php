...
@section('content')
{{-- Main Wrapper with viewport-aware padding --}}
<div class="container-fluid py-4 py-md-5">
    
    <div class="row justify-content-center">
        {{-- This column size ensures it fits perfectly in a dashboard with a sidebar --}}
        <div class="col-12 col-xxl-8 col-xl-9 col-lg-10">
            
            {{-- Navigation & Header Section --}}
            <div class="text-center mb-5">
                <a href="{{ route('student.quizzes.topics_diff', [$subject->id, $difficulty]) }}" 
                ...
                <div class="header-content animate-fade-in">
                    ...
                    <h1 class="display-3 fw-black text-dark mb-3">{{ $topic }}</h1>
                    
                    {{-- Status Indicator Bar --}}
                    <div class="d-flex justify-content-center align-items-center gap-3">
                        @php $color = $difficulty == 'Easy' ? 'success' : ($difficulty == 'Medium' ? 'warning' : 'danger'); @endphp
                        <div class="flex-grow-1 border-top border-2 opacity-25 d-none d-sm-block"></div>
                        <span class="badge bg-{{ $color }} bg-opacity-10 text-{{ $color }} border border-{{ $color }} rounded-pill px-4 py-2 fs-6 fw-black">
                            <i class="fas fa-signal me-2"></i>{{ strtoupper($difficulty) }} LEVEL
                        </span>
                        <div class="flex-grow-1 border-top border-2 opacity-25 d-none d-sm-block"></div>
                    </div>
                </div>
            ...
            <div class="card border-0 shadow-lg rounded-5 mb-4 overflow-hidden position-relative hover-lift transition-all">
                ...
                {{-- Dynamic Vertical Accent --}}
                <div class="position-absolute top-0 start-0 h-100" style="width: 10px; background-color: var(--bs-{{ $color }});"></div>
                ...
                <div class="card-body p-4 p-md-5">
                    <div class="row align-items-center g-4">
                        ...
                        <div class="col-lg-8 text-center text-lg-start">
                            ...
                            <h2 class="fw-black text-dark mb-2">{{ $quiz->title }}</h2>
                            <p class="text-muted fs-5 mb-0">{{ $quiz->description ?: 'Begin this challenge to test your proficiency.' }}</p>
                        </div>
