...
<div class="container-fluid p-0">
    ...
    <ul class="nav nav-pills mb-4 gap-2" id="pills-tab" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <button class="nav-link rounded-pill px-4 {{ $loop->first ? 'active' : '' }} {{ $level == 'Very Easy' ? 'btn-outline-info' : ($level == 'Easy' ? 'btn-outline-success' : ($level == 'Medium' ? 'btn-outline-warning' : ($level == 'Hard' ? 'btn-outline-danger' : 'btn-outline-dark'))) }}"
                    id="pills-{{ $level }}-tab"
                    data-bs-toggle="pill"
                ...
                @if(isset($groupedQuizzes[$level]) && $groupedQuizzes[$level]->count() > 0)
                    ...
                    <div class="col-md-6 col-lg-4">
                        <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden position-relative">
                            {{-- Difficulty Stripe --}}
                            <div class="position-absolute top-0 start-0 w-100" style="height: 5px; background: {{ $level == 'Very Easy' ? '#0dcaf0' : ($level == 'Easy' ? '#22c55e' : ($level == 'Medium' ? '#f59e0b' : ($level == 'Hard' ? '#ef4444' : '#6c757d'))) }};"></div>

                            <div class="card-body p-4 d-flex flex-column">
                                <div class="d-flex justify-content-between mb-3">
                                    <span class="badge {{ $level == 'Very Easy' ? 'bg-info' : ($level == 'Easy' ? 'bg-success' : ($level == 'Medium' ? 'bg-warning' : ($level == 'Hard' ? 'bg-danger' : 'bg-dark'))) }} bg-opacity-10 text-dark border">
                                        {{ $level }}
                                    </span>
