    ...
    <ul class="navbar-nav ml-auto">
      <li class="nav-item dropdown">
          <a class="nav-link" data-toggle="dropdown" href="#" style="cursor: pointer;">
             <span class="font-weight-bold mr-2 text-light">{{ Auth::user()->name }}</span>
    ...
    <div class="sidebar">
      ...
      <nav class="mt-4">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu">
          <li class="nav-item">
            <a href="{{ route('adminreal.dashboard') }}" class="nav-link {{ request()->routeIs('adminreal.dashboard') ? 'active' : '' }}">
...
<div class="modal fade" id="adminProfileModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content bg-dark text-light border border-secondary" style="border-radius: 12px; background-color: #121214 !important;">
            ...
            <form action="{{ route('teachers.update', Auth::user()->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="form-group mb-3">
                        <label class="text-xs text-muted font-weight-bold text-uppercase mb-1">Username</label>
                        <input type="text" name="name" class="form-control bg-transparent text-white border-secondary text-sm" value="{{ Auth::user()->name }}" required>
                    </div>

                    <div class="form-group mb-3">
                        <label class="text-xs text-muted font-weight-bold text-uppercase mb-1">Email Address</label>
                        <input type="email" name="email" class="form-control bg-transparent text-white border-secondary text-sm" value="{{ Auth::user()->email }}" required>
                    </div>
